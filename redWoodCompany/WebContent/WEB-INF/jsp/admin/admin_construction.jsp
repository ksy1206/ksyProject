...
<head>
  ...
  .affix + .container-fluid {
	...
	function save_sympathy(){
        ...
        $.ajax({
            type        : "POST" 
          , async       : true
          , url         : "/admin/save_img_construction.do"
          , data        : params
          , dataType    : "json"
          , timeout     : 30000  
          , cache       : false    
  ...
  function saveCompanyFile(){
	...
	function deletePotolio(constPortpolio_no){
		if(confirm("민우야 진자로 삭제할꺼니?")){
			location.href="<%=cp%>/admin/constructionPortpolio_delete.do?constPortpolio_no="+constPortpolio_no;
		}
	}
	</script>
  ...
  <h1>CG 이미지 올리기</h1>
    ...
    <div class="row">
    	<c:forEach var="data" items="${potolio_list}">
	        <div class="col-md-3 img-portfolio" onclick="deletePotolio('${data.constPortpolio_no}')">
